...
daymet_one = download_daymet(site = field$site[1],
                             ...
                             simplify = T # to obtain data output in table format
                             )

daymet_one

# 2555/365 = 7 weather variable (it was pivoted longer by default)
```
...
# pivoted wider made it possible to have 365 data count based on 7 data variables
```
How many rows above? Why?  
    ...
    weather = pmap(
      ...
      function(.y, .site, .lat, .lon)
        download_daymet(
          ...
          simplify = T, # get a table format output
          silent = T
...
daymet_all_unnest = daymet_all %>%
   unnest(cols = weather) %>% pivot_wider(
     names_from = measurement,
     values_from = value # from long format to wide format
   ) %>% janitor::clean_names()
